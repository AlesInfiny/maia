...
その後、[`#!java @ConfigurationProperties` :material-open-in-new:](https://spring.pleiades.io/spring-boot/reference/features/external-config.html#features.external-config.typesafe-configuration-properties){ target=_blank } を利用して `application.properties` に記述した Cookie の設定値を読み込みます。
