...
また、 H2 Database のデータベース管理ツールである H2 Console のスタンドアロン版を使用してデータベースを操作する際も、アプリケーションと H2 Console が同一のデータベースへ同時にアクセスします。そのため、この場合も H2 Database はサーバーモードで起動する必要があります。
