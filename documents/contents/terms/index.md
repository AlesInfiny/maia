...
本サイトを使用された場合、本ライセンスに同意したとみなします。

## ライセンス {#license}

BIPROGY 株式会社およびすべてのコントリビューターは、 AlesInfiny Maia のソースコードを除く Web サイトを構成するコンテンツに[クリエイティブ・コモンズ表示 4.0 国際ライセンス](#cc-by-4.0)のライセンスを付与します。
コンテンツとは、 Web サイトに公開されているドキュメント、画像などです。
また AlesInfiny Maia のサンプルアプリケーションや、ドキュメント内のコードに、 [Apache License, Version 2.0](#apache-license-v2.0) のライセンスを付与します。
ライセンスの条項に従う限り、個人、商用とも無料で使用できます。
ライセンスの本文は、次節を参照してください。
...
- :ok: 本サイト、および本サイトのコンテンツ、ソースコードへの参照を示す場合。
