...
### JavaScript や Python などのスクリプト言語に書き換える {#script-language}
...
スクリプト言語に置き換える場合には、クラウドの FaaS サービスを利用することを推奨します。
バッチ処理を FaaS サービスに配置することで、スクリプト言語による起動時間短縮の他にも多くのメリットがあげられます。

**実行時間の高速化**
クラウドサービスは大量のコンピューティングリソースを提供しており、これによりバッチ処理の実行時間も高速化が期待できます。
