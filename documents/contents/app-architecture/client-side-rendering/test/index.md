...
- 単体テスト ( UT0 )
    ...
    アプリケーションが仕様通りに実装されているか、単一のアプリケーションに閉じた範囲で確認するテストです。
    バックエンドアプリケーションでは、クラスやメソッドなど、テスト可能な最小単位を対象とします。
    フロントエンドアプリケーションでは、ロジックやコンポーネントの単位に加えて、それらを結合して実現する機能やユースケースの単位も対象とします。
    いずれの場合も、他のアプリケーションと結合しない範囲でテストします。
    画面のデザインの再現性は、 UT0 の範囲外です。
...
- 結合テスト ( ITa )
    ...
    フロントエンドアプリケーションのテストは、バックエンドアプリケーションと結合したユースケース単位で行います。
