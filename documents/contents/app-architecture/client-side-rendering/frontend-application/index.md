...
```text title="プロジェクトのフォルダー構成全体像" linenums="0"
src/
├─ feature/          --------- (参考) create-vue で作成したデフォルト
│  ├─ assets/ 
│  ├─ components/ 
│  ├─ router/
│  ├─ stores/
...
│  ├─ validations/ ----------- ★
│  ├─ views/ ----------------- 
├─ common/        ------------ システム共通機能 各 feature から readonly にする
│  ├─ __tests__　 ------------ ★1機能に1フォルダでよくないか説
│  ├─ components/　----------- コンポーネント
│  ├─ composable/ ------------ コンポーザブル
│  ├─ plugins/ --------------- プラグイン
│  ├─ services/ --------------
...
│  ├─ validations/ ----------- 
│  ├─ views/ ----------------- 
├─ core/          ------------ 業務共通機能 各 feature から read/write する
│  ├─ __tests__　 ------------ ★1機能に1フォルダでよくないか説
│  ├─ api-client/ ------------ ★axios interceptor の処理　システム共通？
│  ├─ components/　----------- コンポーネント
│  ├─ composable/ ------------ コンポーザブル
│  ├─ generated/ ------------- OpenAPI Generator の吐き先　システム共通？
│  ├─ plugins/ --------------- プラグイン
│  ├─ router/　--------------- うまく各 feature から定義を収集して再 export
│  ├─ stores/ ---------------- ストア
│  ├─ locales/ --------------- i18n とメッセージ一覧
├─ feature/ ------------------ 機能
│  ├─ __tests__　 ------------ ★1機能に1フォルダでよくないか説
│  ├─ assets/ ---------------- ■画像
│  ├─ components/ ------------ ■コンポーネント
│  ├─ composable/ ------------ ■コンポーザブル
│  ├─ plugins/ --------------- ■Vue.js のプラグイン
│  ├─ router/ ---------------- ★ export する
│  ├─ services/ -------------- ■サービス
│  ├─ stores/ ---------------- ■ストア
│  ├─ types/ ----------------- ■型
│  ├─ validations/ ----------- ★スキーマ以外になにかあるか？
│  ├─ views/ ----------------- ■各機能の業務画面
├─ mock ---------------------- ？src の下か？
├─ e2e ----------------------- ？ここか？
├─ public
