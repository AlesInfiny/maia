...
メッセージコードはそのメッセージの内容を簡潔に表す文字列とします。
...
## フロントエンドアプリケーション {#frontend-application}
...
### メッセージの管理単位 {#frontend-management-unit}
...
### メッセージのファイル管理 {#frontend-message-file-management}
...
## バックエンドアプリケーション {#backend-application}
...
ビジネスロジックで利用する業務メッセージと共通処理として利用する共通メッセージを分割して管理します。
...
### メッセージのファイル管理 {#backend-message-file-management}

以下のように、業務メッセージと共通メッセージを分割して各サブプロジェクトで管理します。
