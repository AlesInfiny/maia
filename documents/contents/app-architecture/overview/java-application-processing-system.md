...
ヘルスチェックの目的は、ロードバランサ（以下、 LB と記載する）がサーバーの稼働状況を監視することです。
...
LB が行うヘルスチェックには、使用するプロトコルレイヤの違いによって、以下に示すような種類で分類できます。
...
ここで、 Layer 3 や Layer 4 のプロトコルレイヤで行うヘルスチェックは、作成した Web アプリケーションの稼働状況まで確認できません。
