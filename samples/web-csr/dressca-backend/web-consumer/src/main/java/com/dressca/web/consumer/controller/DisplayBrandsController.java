 ...
 */
@RestController
@Tag(name = "DisplayBrands", description = "陳列ブランドの情報にアクセスする API です。")
...
public class DisplayBrandsController {
   ...
   * @return 陳列ブランドの一覧。
   */
  @Operation(summary = "陳列ブランドの一覧を取得します。",
      description = "陳列ブランドの一覧を取得します。")
  @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "成功。",
      content = @Content(mediaType = "application/json",
          array = @ArraySchema(
              schema = @Schema(implementation = GetDisplayBrandsResponse.class))))})
  @GetMapping()
  public ResponseEntity<List<GetDisplayBrandsResponse>> getDisplayBrands() {
    List<GetDisplayBrandsResponse> brands = this.service.getBrands().stream()
