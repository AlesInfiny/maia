...
public class BasketItemController {
  ...
  public ResponseEntity<GetBasketItemsResponse> getBasketItems(HttpServletRequest req) {
    ...
    for (BasketItemApiModel item : basketDto.getBasketItems()) {
      item.setDisplayItemSummary(
          this.getDisplayItemResponse(item.getDisplayItemId(), displayItems));
    }
  ...
  /**
   * 買い物かごアイテム内の数量を変更します。買い物かご内に存在しない掲載品 ID は指定できません。
   * 
   * <p>この API では、買い物かご内に存在する商品の数量を変更できます。
   * 買い物かご内に存在しない掲載品 ID を指定すると HTTP 400 を返却します。
   * またシステムに登録されていない掲載品 ID を指定した場合も HTTP 400 を返却します。</p>
   * 
   * @param putBasketItems 変更する買い物かごアイテムのデータリスト。
   * @return なし。
   */
  @Operation(summary = "買い物かごアイテム内の数量を変更します。",
      description = "買い物かごアイテム内の数量を変更します。買い物かご内に存在しない掲載品 ID は指定できません。<br>"
          + "この API では、買い物かご内に存在する商品の数量を変更できます。買い物かご内に存在しない掲載品 ID を指定すると HTTP 400 を返却します。<br>"
          + "またシステムに登録されていない掲載品 ID を指定した場合も HTTP 400 を返却します。")
  @ApiResponses(
   ...
   * 買い物かごに商品を追加します。
   * 
   * <p>この API では、システムに登録されていない掲載品 ID を指定した場合 HTTP 400 を返却します。
   * また買い物かごに追加していない掲載品を指定した場合、その商品を買い物かごに追加します。
   * すでに買い物かごに追加されている掲載品を指定した場合、指定した数量、買い物かご内の数量を追加します。</p>
   * 
   * <p>買い物かご内の掲載品の数量が 0 未満になるように減じることはできません。 計算の結果数量が 0 未満になる場合 HTTP 500 を返却します。</p>
   * 
   * @param postBasketItem 追加する商品の情報。
   * @return なし。
   */
  @Operation(summary = "買い物かごに商品を追加します。",
      description = "買い物かごに商品を追加します。<br>" + "この API では、システムに登録されていない掲載品 ID を指定した場合 HTTP 400 を返却します。"
          + "また買い物かごに追加していない掲載品を指定した場合、その商品を買い物かごに追加します。"
          + "すでに買い物かごに追加されている掲載品を指定した場合、指定した数量、買い物かご内の数量を追加します。<br>"
          + "買い物かご内の掲載品の数量が 0 未満になるように減じることはできません。計算の結果数量が 0 未満になる場合 HTTP 500 を返却します。")
  @ApiResponses(
  ...
  /**
   * 買い物かごから指定した掲載品 ID の商品を削除します。
   * 
   * <p>displayItemId には買い物かご内に存在する掲載品 ID を指定してください。
   * 掲載品 ID は 1 以上の整数です。
   * 0 以下の値を指定したり、整数値ではない値を指定した場合 HTTP 400 を返却します。
   * 買い物かご内に指定した掲載品の商品が存在しない場合、 HTTP 404 を返却します。</p>
   * 
   * @param displayItemId 掲載品 ID 。
   * @return なし。
   */
  @Operation(summary = "買い物かごから指定した掲載品 ID の商品を削除します。",
      description = "買い物かごから指定した掲載品 ID の商品を削除します。<br>"
          + "displayItemId には買い物かご内に存在する掲載品 ID を指定してください。掲載品 ID は 1 以上の整数です。"
          + "0 以下の値を指定したり、整数値ではない値を指定した場合 HTTP 400 を返却します。"
          + "買い物かご内に指定した掲載品の商品が存在しない場合、 HTTP 404 を返却します。")
  @ApiResponses(
      value = {@ApiResponse(responseCode = "204", description = "成功。", content = @Content),
          @ApiResponse(responseCode = "400", description = "リクエストエラー。",
              content = @Content(mediaType = "application/problem+json",
                  schema = @Schema(implementation = ProblemDetail.class))),
          @ApiResponse(responseCode = "404", description = "買い物かご内に指定した掲載品 ID がありません。",
              content = @Content(mediaType = "application/problem+json",
                  schema = @Schema(implementation = ProblemDetail.class)))})
  @DeleteMapping("{displayItemId}")
  public ResponseEntity<?> deleteBasketItem(@PathVariable("displayItemId") long displayItemId,
      HttpServletRequest req) {
    String buyerId = req.getAttribute(WebConstants.ATTRIBUTE_KEY_BUYER_ID).toString();
  ...
  private DisplayItemSummaryApiModel getDisplayItemResponse(long displayItemId,
      List<DisplayItem> displayItems) {
    DisplayItem displayItem = displayItems.stream().filter(item -> item.getId() == displayItemId)
