 ...
 *
 * バックエンドのアセット管理コンテキスト（applicationmodules/assetsmanagement）に対応します。
 * 利用元がカタログ管理コンテキストだけであっても、バックエンドが独立したコンテキストとして
 * 切っているため、フロントエンドでも独立させます。
 * 業務概念を持たない表示整形（`currencyHelper` など）は system-common に配置します。
 * @returns アセット操作用の関数群
 */
