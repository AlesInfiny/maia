# dressca-cms 認証方式の Microsoft Entra External ID (CIAM) 移行プラン

## Context

dressca-cms は Spring Boot 4.0.7 (Spring Security 7) + Thymeleaf の SSR 型 Web アプリで、現在は独自フォーム認証を実装している:

- `web/src/main/java/com/dressca/cms/web/controller/LoginController.java` が email/password から `UsernamePasswordAuthenticationToken` を生成し `AuthenticationManager.authenticate()` → HTTP セッションへ保存
...
## 前提作業: Entra External ID テナント側の手動セットアップ

コード変更の前提として Microsoft Entra 管理センターで実施:

1. 外部テナントを用意し、テナント ID・サブドメインを控える
...
6. **サインアップ/サインイン ユーザーフロー**作成: 収集属性とアプリケーションクレームの両方で Display Name / Email Address を有効化し、アプリ登録を関連付け
7. **疎通確認**: `https://{subdomain}.ciamlogin.com/{tenant-id}/v2.0/.well-known/openid-configuration` を開き、**レスポンスの `issuer` 値をそのまま `issuer-uri` に使う**(サブドメイン名でなくテナント GUID 形式で返ることが多く、不一致だと issuer 検証で失敗する — 最重要の落とし穴)
...
## 2. SecurityConfig の書き換え

`web/src/main/java/com/dressca/cms/web/security/SecurityConfig.java` を全面書き換え(`PasswordEncoder` / `AuthenticationManager` / `DaoAuthenticationProvider` Bean、`UserDetailsServiceImpl` 依存、`formLogin` / `exceptionHandling` をすべて削除):
...
- `/oauth2/authorization/*` と `/login/oauth2/code/*` は専用フィルターが認可判定より前段で処理するため permitAll 不要
- クライアント登録が 1 件のみなら、未認証アクセスは自動的に Entra のサインイン画面へ直行する(中間ログインページなし)
- **元 URL への復帰は標準の `HttpSessionRequestCache`(saved request)が代替**するため、`ReturnUrlQueryAppendingEntryPoint.java` と returnUrl 機構は削除できる。saved request がない場合のデフォルト遷移先 `/` は `HomeController` が処理する
- CSRF・セッション固定対策は Spring Security デフォルトのまま

## 3. OAuth2 クライアント設定(properties)
...
- `redirect-uri` は未指定でよい(デフォルトの `{baseUrl}/login/oauth2/code/{registrationId}` が実行環境に自動追従)
- シークレットは環境変数 `ENTRA_CLIENT_SECRET` で注入。ローカルは `.vscode/launch.json` が参照する `.env` に書ける(`.env` が `.gitignore` に含まれていないため追記する)
...
## 4. プリンシパル参照の置き換え(`UserDetails` → `OidcUser`)
...
- **`authentication/` モジュール全体**(UserDetailsServiceImpl、UserDetailsImpl、UserRepository、MyBatisUserRepository、Mapper/Entity、schema-authentication.sql、data-authentication.sql、i18n/authentication/messages.properties、mybatisGeneratorConfig.xml、application-*.properties、UserDetailsServiceImplTest、build.gradle)
...
参照破壊チェック済み: `com.dressca.cms.authentication` への参照は SecurityConfig と WebApplication のみ。`authentication.login.*` の i18n キー参照は削除対象ファイルに閉じる。i18n は `I18nConfig` の glob スキャンのため設定変更不要。`schema-*.sql`/`data-*.sql` も glob 読み込みのためモジュール削除で自動的に消える。DB の FK 参照なし。PostgreSQL/H2 ドライバは announcement モジュールも宣言済みで prd 接続に影響なし。

## 6. テスト

- 既存テストは `web/src/test/java/com/dressca/cms/web/WebApplicationTests.java`(contextLoads)のみ — §3 の ut 設定で起動が通る
- 追加(推奨): `SecurityMockMvcRequestPostProcessors.oidcLogin().idToken(token -> token.claim("email", ...))` で `AnnouncementController` / `AuthenticatedUserModelAdvice` の OidcUser 経路を実テナントなしで検証。未認証アクセスは 302 → `/oauth2/authorization/entra` を期待値にする

## 実装順序と検証

実装順: ビルド設定(§1)→ 削除(§5)→ SecurityConfig(§2)→ プリンシパル置換(§4)→ properties(§3)→ テスト(§6)

1. `./gradlew build` — コンパイル・テスト・checkstyle/spotbugs 通過(新 SecurityConfig も既存同様の Javadoc スタイルを維持)
2. テナント値を環境変数に設定し `./gradlew :web:bootRunDev` で起動(起動時に OIDC ディスカバリーへ HTTPS アクセスが発生 — 到達不能だと起動失敗。プロキシ環境では JVM プロキシ設定が必要)
3. `http://localhost:8083/top` → Entra サインイン画面へリダイレクト → サインアップ/サインイン → `/top` 復帰、ヘッダーに表示名/メール表示
...
3. **userinfo 呼び出し**: `profile`/`email` スコープがあると `OidcUserService` が Graph の userinfo(`https://graph.microsoft.com/oidc/userinfo`)を呼び、CIAM で 401 になる事例あり。発生したら `OidcUserService` Bean で `setRetrieveUserInfo(request -> false)` を設定(必要クレームは ID トークンに載る)
...
6. **本番 DB の後始末**: prd は `spring.sql.init.mode=never` のため `application_users` テーブルの DROP は手動の運用作業として別途計画
7. **actuator**: `/actuator/health` も Entra へ 302 になる(現行も /account/login へ 302 なので劣化ではない)。インフラのヘルスチェックが要るなら `requestMatchers("/actuator/health").permitAll()` を検討
